...
export async function get() {
  ...
  const strictSSL = config.get<boolean>('GithubCopilotBus.proxy.strictSSL')
  const enable = config.get<boolean>('GithubCopilotBus.proxy.enable')
  const networkProxy: {
    host?: string
    port?: number
    username?: string
  ...
  if (enable) {
    const host = config.get<string>('GithubCopilotBus.proxy.host') || ''
    ...
    if (hostname && port) {
      ...
      const user = config.get<string>('GithubCopilotBus.proxy.user') || ''
